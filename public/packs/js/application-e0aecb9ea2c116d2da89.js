...
mapboxgl.accessToken = 'pk.eyJ1Ijoia3JwYXVsIiwiYSI6ImNrMnV4b2w5dTFhbnQzaG50YmppcHVhdWcifQ.rwTKRa2OQmqweeps8ZXELw'; // Global vars
...
function newData(data) {
  ...
  timestampLastUpdate = data.timestamp; // Update numerical data
  ...
  $("#longitude-value")[0].innerText = lng.toFixed(2); // Add pair to lines

  latlngPairs.push([lat, lng]); // Add altitude

  altitudePoints.push(alt); // Add a line to the map

  addMapLines(latlngPairs); // Update Altitude Chart
...
function allData(data) // Fills in all data packets from /all request
{
  ...
  data.forEach(function (packet) {
    ...
    var lng = parseFloat(packet.longitude); // Insert into arrays
  ...
  var last = data.last();
  console.log("jumping to: ", [last.latitude, last.longitude]);
  map.jumpTo({
...
$(document).ready(function () {
  ...
  map = new mapboxgl.Map({
    ...
    // style URL
    center: [0, 0],
    // [initialMapData.last().latitude, initialMapData.last().longitude], 
    zoom: 5
  });
  map.on('load', function () {
    // Create altitude chart
    createAltChart(); // Let the window check for an update every 5 seconds with new data
  ...
  resetTimeSinceLastUpdate(); // Last time update

  window.setInterval(updateTimeSinceLastUpdate, 1000);
});
/*******************
    Utility Methods 
*******************/
...
*/
{
  ...
  if (!args) return; // If given as part of an object, or as just the value

  var value = args.value || args;
  var date = new Date(value * 1000);
  console.log("".concat(date.getMinutes(), ":").concat(date.getSeconds()));

  switch (mode) {
    default:
    case 0:
      return "".concat(date.getHours(), "h ").concat(date.getMinutes(), "m");

    case 1:
      return "".concat(date.getMinutes(), "m ").concat(date.getSeconds(), "s");
  }
}
...
function toolTipFormatter(e) {
  ...
  for (var i = 0; i < e.entries.length; i++) {
    console.log("x:: ", e.entries[i].dataPoint.x); // Create html for this tooltip entry

    var current = "<strong>\n            ".concat(formatUnixTime(e.entries[i].dataPoint.x, mode = 1), "\n            </strong>, \n            ").concat(e.entries[i].dataPoint.y, "m"); // Append to overarching string
...
/******/ });
//# sourceMappingURL=application-e0aecb9ea2c116d2da89.js.map